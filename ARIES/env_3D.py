...
class TumorEnv(gym.Env):
    ...
    def __init__(
        ...
        if discrete_actions:
            self.action_space = spaces.Discrete(7 + 2*self.targetSize)
        else:
            self.action_space = spaces.Box(0, 1, (6 + 2*self.targetSize + 1,))
        self.l = [2,1]
        if self.n_obs == 3 :
        ...
        if self.n_obs == 4 :
            self.doseMaps[0] = 100*(self.perfect_DM[self.energy] - self.DM_i[self.energy])/self.SignalLength
            #self.doseMaps[1] = self.beam_pos
    ...
    def step(self, action):
        ...
        shift = np.zeros(3)
        shift[0] = 0
        shift[2] = self._tumor_position[2] - self.ref_position[2]
        ...
        if action == 0: # move to the left
            ...
            self.DM_i_noisy[self.energy, x_ - self.le : x_ + self.le +1 , y_ - self.le : y_ + self.le + 1] += self.gauss
            # if self.mode != -1 :
            x_in_ref = int(self._beam_position[0] - self._tumor_position[1] + (self.num_row/2))
            y_in_ref = int(self._beam_position[1] - self._tumor_position[2] + (self.num_col/2))
            ...
            x_in_ref_noisy = int(self._beam_position[0] - self.noisy_tumor_position[1] + (self.num_row/2))
            y_in_ref_noisy = int(self._beam_position[1] - self.noisy_tumor_position[2] + (self.num_col/2))
            if x_in_ref_noisy >= self.le and x_in_ref_noisy < (self.num_row-self.le) :
                if y_in_ref_noisy >= self.le and y_in_ref_noisy < (self.num_col-self.le) :
                    self.DMi_inRef_noisy[self.energy, x_in_ref_noisy - self.le : x_in_ref_noisy + self.le +1 , y_in_ref_noisy - self.le : y_in_ref_noisy + self.le + 1] += self.gauss
        elif action > 5 : #5 étant ne rien faire 6 7 8 9 10 11 12
            self.energy = int(int(self.depth/2) + 6 + self.targetSize - action)
        ...
        if self.n_obs == 4 :
            self.doseMaps[0] = 100*(self.perfectDM_inref[self.energy]-self.DMi_inRef_noisy[self.energy])/self.SignalLength
    ...
    def observeEnvAs2DImage(self, pos = None, dose_q = 1, form = "ball", targetSize = 2):
        ...
        if self.form == "cube":
            target = (dose_q/2)*cube(2*(targetSize+1) + 1) #square(2*self.targetSize + 1) #disk(self.targetSize) #[1, 1]#
        if self.form == "ball":
            target = (dose_q/2)*ball(targetSize+1)
        if pos is not None:
        ...
        envImg1[targetCenterInPixels[0] - int(targetSize + 1): targetCenterInPixels[0] + int(targetSize + 2),
        ...
        if self.form == "cube":
            target = (dose_q/2)*cube(2*(targetSize) + 1) #square(2*self.targetSize + 1) #disk(self.targetSize) #[1, 1]#
        if self.form == "ball":
            target = (dose_q/2)*ball(targetSize)
        if pos is not None:
            targetCenter = ref_position
        # else:
        #     targetCenter = np.array(self.breathingSignal.breathingSignal[self.curTimeStep, 0], self.breathingSignal.breathingSignal[self.curTimeStep, 1])
