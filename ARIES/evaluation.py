...
def evaluate_over_treatment_daily(model, env, n_eval_episodes=30, deterministic=True, epoch=0):
    ...
    actions = np.zeros((n_eval_episodes*env.SignalLength,1))
    rewards = np.zeros((n_eval_episodes*env.SignalLength,1))
    real_pos = np.zeros((n_eval_episodes*env.SignalLength,2))
    beam_pos = np.zeros((n_eval_episodes*env.SignalLength,2))
    noisy_pos = np.zeros((n_eval_episodes*env.SignalLength,2))
    Accum_DM = np.zeros((env.num_row, env.num_col))
    d_meanOAR = np.zeros(n_eval_episodes)
    d_meanAutour = np.zeros(n_eval_episodes)
    d_meanPTV= np.zeros(n_eval_episodes)
    ...
    for i in range(n_eval_episodes):
        ...
        while not done:
            ...
            # because we are using vectorized env
            obs, reward, done, info = env.step(action)
            actions[(i*env.SignalLength)+j] = action
            rewards[(i*env.SignalLength)+j] = reward
            real_pos[(i*env.SignalLength)+j] = env._tumor_position
            noisy_pos[(i*env.SignalLength)+j] = env.noisy_tumor_position
            beam_pos[(i*env.SignalLength)+j] = env._beam_position
            #tumor_recovery.append(info)
            episode_rewards.append(reward)
            j = j + 1 
...
def plot_treatment_3D(env, n_eval_episodes, D98_PTV_treat, D80_PTV_treat, D30_OAR_treat, mean_OAR_treat, Accum_DM, epoch):
    ...
    # ax2.set_xlim3d([0.0, env.num_row])
    # ax2.set_xlabel('X')

    # ax2.set_ylim3d([0.0, env.num_col])
    # ax2.set_ylabel('Y')

    # ax2.set_zlim3d([0.0, env.depth])
    # ax2.set_zlabel('Z')
